...
@dataclass
class RuleFit:
    """The minimum a rule needs to be extracted and then tested.

    :func:`validate_rules` was written against the study's ``FoldFit``, which
    also carries a trained XGBoost model. Phase rules have no such model -- the
    tree *is* the model -- so this supplies the same five fields and nothing
    else, letting one validation path serve both.
    """
...
def phase_tree_rules(
    ...
    """Shallow tree fitted to *measured* release, with time excluded as an input.

    Two things separate this from :func:`surrogate_tree_rules`.

    **It is not a surrogate.** The target is the release actually measured, not
    the trained predictor's output, so a rule that comes out is a claim about
    the data rather than a description of XGBoost's behaviour. That makes it
    directly testable on held-out drugs -- which is the point -- and it removes
    the awkwardness of a surrogate that faithfully reproduces a model's errors.

    **Time is not among the columns.** ``fit.X_train`` is expected to hold
    formulation and drug descriptors only, restricted to rows from a single
    release phase (see :mod:`fuzzypharma.phases`). Because time cannot be split
    on, the tree cannot return the trivial "later means more released" rule and
    every split it makes is on something a formulator controls. Holding time
    fixed by restriction rather than by including and then discounting it is
    what makes the resulting effect sizes interpretable.

    A tree is used here as an interpretable **rule-discovery mechanism over a
    fuzzy representation**. It is an ordinary crisp CART tree; calling the
    result a fuzzy inference system would be wrong, and the rules it produces
    are candidates until :func:`validate_rules` has been run on them.
    """
    tree = DecisionTreeRegressor(
        max_depth=max_depth, min_samples_leaf=min_samples_leaf, random_state=seed
    ).fit(fit.X_train, fit.y_train)

    method = label or f"Phase tree (depth {max_depth})"
    report = {
        "fold": fit.fold,
        "method": method,
        "n train rows": int(len(fit.X_train)),
        "n valid rows": int(len(fit.X_valid)),
        "train R2": float(tree.score(fit.X_train, fit.y_train)),
        "valid R2": float(r2_score(fit.y_valid, tree.predict(fit.X_valid)))
        if len(fit.y_valid) > 1
        else np.nan,
        "n leaves": int(tree.get_n_leaves()),
    }

    names = list(fit.X_train.columns)
    structure = tree.tree_
    baseline = float(np.mean(fit.y_train))
    rows = []
    ...
    def walk(node: int, path: list[Condition]) -> None:
        if structure.children_left[node] == -1:
            collapsed = simplify(path)
            if not collapsed:
                return  # a stump: no condition to state
            leaf_mean = float(structure.value[node].ravel()[0])
            rows.append(
                {
                    "fold": fit.fold,
                    "method": method,
                    ...
                    "conditions": collapsed,
                    "coef": leaf_mean - baseline,
                    "train support": float(
                        structure.n_node_samples[node] / structure.n_node_samples[0]
                    ),
                    "importance": abs(leaf_mean - baseline),
                    "leaf mean release (train)": leaf_mean,
                }
            )
            return
        feature = names[structure.feature[node]]
...
def tree_text(
    fit: RuleFit, max_depth: int = 3, min_samples_leaf: int = 30, seed: int = SEED
) -> str:
    """The same tree rendered by ``export_text``, for the technical appendix.

    The readable rule strings elsewhere in this module paraphrase membership
    thresholds into words. That paraphrase is for the write-up; this keeps the
    exact numerical criterion available so a reader can check it.
    """
    ...
    return export_text(tree, feature_names=list(fit.X_train.columns), decimals=3)
...
#: Smallest held-out release difference worth calling a finding, in fractional
#: release. 0.05 is five percentage points; below that a "difference" is inside
#: the measurement scatter of curves pooled from 113 separate publications.
#: Conservative and declared in advance, not chosen after seeing the results.
MIN_EFFECT = 0.05

#: Evidence tiers. A rule moves up only by surviving the tier below it.
CANDIDATE = "candidate"
...
def classify_evidence(
    ...
    """Assign each rule an evidence tier, and say why it stopped there.

    The distinction the proposal demands, made mechanical:

    ``candidate``
        A tree produced it. That is all. It has not passed the held-out test, so
        it is a hypothesis and must never be written down as a formulation
        conclusion.
    ``replicated``
        It passed coverage, direction and interval on its own held-out fold, and
        an equivalent pattern did the same in at least ``min_folds`` folds. Each
        fold holds out different drugs, so this is not one result counted twice.
    ``validated finding``
        Replicated *and* the held-out effect is at least ``min_effect``. A rule
        can be statistically reliable and still describe a difference too small
        to matter; this tier is the one that requires it to be both.
    ``rejected``
        Failed the held-out test on its own fold.

    Even the top tier is an empirical association across 321 literature-sourced
    formulations. It is not a mechanism and carries no causal claim.
    """
    out = validated.copy()
    if out.empty:
        out["evidence"] = []
    ...
    folds_per_signature = (
        out[out["validated"]].groupby("signature")["fold"].nunique().to_dict()
    )
    directions = (
        out[out["validated"]]
        .groupby("signature")["valid effect"]
        .apply(lambda s: len(set(np.sign(s[np.isfinite(s)]))) == 1)
        .to_dict()
    )

    tiers, reasons = [], []
    for _, rule in out.iterrows():
        signature = rule["signature"]
        n_folds = folds_per_signature.get(signature, 0)
        consistent = directions.get(signature, False)
        effect = abs(rule["valid effect"]) if np.isfinite(rule["valid effect"]) else 0.0
        ...
        if not rule["validated"]:
            ...
            tiers.append(REJECTED)
            reasons.append("failed held-out test: " + ", ".join(failed))
        elif n_folds < min_folds or not consistent:
            tiers.append(CANDIDATE)
            reasons.append(
                f"passed its own fold but replicated in only {n_folds}/{min_folds} "
                f"required folds" + ("" if consistent else " (direction inconsistent)")
            )
        elif effect < min_effect:
            tiers.append(REPLICATED)
            reasons.append(
                f"replicated in {n_folds} folds but held-out effect {effect:.3f} "
                f"< {min_effect} threshold"
            )
        else:
            tiers.append(FINDING)
            reasons.append(f"replicated in {n_folds} folds; effect {effect:.3f}")

    out["evidence"] = tiers
    out["evidence reason"] = reasons
    out["folds replicated"] = [
        folds_per_signature.get(s, 0) for s in out["signature"]
    ]
    return out


def evidence_counts(classified: pd.DataFrame) -> pd.DataFrame:
    """How many rules reached each tier -- the headline of rule validation."""
    order = [FINDING, REPLICATED, CANDIDATE, REJECTED]
    counts = classified["evidence"].value_counts()
    return pd.DataFrame(
        {"evidence": order, "n rules": [int(counts.get(tier, 0)) for tier in order]}
    )
