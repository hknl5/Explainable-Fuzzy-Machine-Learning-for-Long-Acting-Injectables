...
# --- Alternative target: literature early-release criterion ---------------

#: Fractional release at or below which a formulation counts as slow-releasing
#: under the published criterion.
SLOW_RELEASE_FRACTION = 0.20

#: The day the criterion is evaluated at.
SLOW_RELEASE_DAY = 3.0

SLOW_RELEASE_COL = "Slow Release (<=20% by day 3)"

SLOW_RELEASE_SOURCE = (
    "Interpretable Two-Stage Machine Learning for Early and Full Drug Release "
    "Prediction in PLGA Microspheres, Pharmaceutics 19(5):767 (2026), which "
    "defines slow release as <=20% released within 3 days."
)


def slow_release_class(model_table: pd.DataFrame) -> pd.Series:
    """1 if the formulation released <= 20% by day 3, else 0; NaN if unobserved.

    This is a **different question** from the normalized-AUC class, not a
    replacement for it, and the two must not be compared as if they were the
    same task. The AUC class summarises the *shape* of a curve after its time
    axis has been rescaled to [0, 1], so it says nothing about how many days
    anything took. This criterion is anchored to real elapsed time, which is
    what an early-burst concern is actually about, and it comes from published
    work on this dataset (see :data:`SLOW_RELEASE_SOURCE`) rather than from
    this study.

    It is reported as a separate experiment for exactly that reason. Adopting a
    threshold because it produces better metrics would be choosing the question
    to fit the answer; this one is adopted because a source defines it.

    Formulations whose curve does not reach day 3 return NaN rather than a
    guess -- ``Release_72h`` is NaN there because interpolation refuses to
    extrapolate.
    """
    day3 = model_table["Release_72h"]
    return pd.Series(
        np.where(day3.isna(), np.nan, (day3 <= SLOW_RELEASE_FRACTION).astype(float)),
        index=model_table.index,
        name=SLOW_RELEASE_COL,
    )
